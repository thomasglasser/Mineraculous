...
public abstract class LookToolClientExtensions<T extends BlockEntityWithoutLevelRenderer & LookRenderer> implements IClientItemExtensions {
     ...
     * 
     * @param player The player providing the look data
     * @param stack  The stack providing the look data
     * @return The look data
     */
    public abstract LookData getLookData(Player player, ItemStack stack);
    ...
    public ResourceLocation getScopeOverlayTexture(ItemStack stack) {
        ...
        if (owner != null && level != null && level.getEntities().get(owner) instanceof Player player) {
            texture = LookManager.getLookAsset(player, getLookData(player, stack), context, LookAssetTypes.SCOPE_TEXTURE);
            if (texture != null)
                return texture;
        }
        texture = LookManager.getDefaultLook(getDefaultLookId(stack)).getAsset(context, LookAssetTypes.SCOPE_TEXTURE);
