...
public class CatStaffRenderer extends GeoItemRenderer<CatStaffItem> {
    ...
    public static void renderCatStaffPerch(Player player, PoseStack poseStack, MultiBufferSource bufferSource, int light, float partialTicks) {
        ...
        if (catStaffPerchRender) {
            ...
            //PAW & LINES
            drawPawAndLines(vertexConsumer, pose, player, bodyDirectionF, direction, nLength, player.getEyeHeight(Pose.STANDING) + bodyDirectionF.y);
        }
        poseStack.popPose();
    }
    ...
    public static void renderCatStaffTravel(Player player, PoseStack poseStack, MultiBufferSource bufferSource, int light, float partialTicks) {
        ...
        if (travelCatStaffData.traveling()) {
            ...
            drawCap(vertexConsumer, pose, light, light, bodyDirectionF.x, bodyDirectionF.z, PIXEL * 12, PIXEL * 12, 1f, 1f);
            //PAW & LINES:
            drawPawAndLines(vertexConsumer, pose, player, bodyDirectionF, (int) ((travelCatStaffData.initBodAngle() + 45f) % 360) / 90, top, top - 1);

        }
        poseStack.popPose();
    }

    private static void drawPawAndLines(VertexConsumer vertexConsumer, PoseStack.Pose pose, Player player, Vector3f bodyDirectionF, int direction, float nLength, float eyeY) {
        //PAW:
