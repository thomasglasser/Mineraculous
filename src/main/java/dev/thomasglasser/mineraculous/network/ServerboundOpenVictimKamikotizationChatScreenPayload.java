...
public record ServerboundOpenVictimKamikotizationChatScreenPayload(UUID targetId, KamikotizationData kamikotizationData) implements ExtendedPacketPayload {
    ...
    public void handle(Player player) {
        if (player.level().getPlayerByUUID(targetId) instanceof ServerPlayer target) {
            player.getData(MineraculousAttachmentTypes.ABILITY_EFFECTS).withFaceMaskTexture(kamikotizationData.kamikoData().faceMaskTexture()).save(player, true);
            TommyLibServices.NETWORK.sendToClient(new ClientboundOpenVictimKamikotizationChatScreenPayload(player.getUUID(), kamikotizationData), target);
